...
class Person(object):
    ...
    def generate_bookings(self, time,rates_table_np):
        ...
        if len(self.possible_start_dates)>1:
            ...
            start_time=self.possible_start_dates[0]
            end_time = self.possible_end_dates[0]
            lead_times = (start_time - time).days #/ dt.timedelta(days=1)
        
            best_bookable_budgets_np = None
            list_rates = list(rates_table_np[:,2])
            if (start_time in list_rates) == True:
                where =list(rates_table_np[:,2]).index(start_time)
                #print(list(rates_table_np[:,2]))
                budgets_np = rates_table_np[where]
            else:
                return None

            min_budget_np=budgets_np[3]
            hotelroom_id_np = budgets_np[1]
            merchant_id_np = budgets_np[0]
                
            new_df_np = np.append(budgets_np,[start_time,end_time])
