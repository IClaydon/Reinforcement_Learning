...
class BaseRevenueManager(object):
    ...
    _unavailable_rooms = attr.ib(default={})

    def get_price(self, time, reserved_night_date, hotelroom_id):
        pass

    #@profile
    def get_rates_table_for_reserved_night_date(self, time, reserved_night_date):
        prices = []
        hotelroom_ids = []
        for hotelroom_id in self.hotel.available_rooms:
            price = self.get_price(time, reserved_night_date, hotelroom_id)
