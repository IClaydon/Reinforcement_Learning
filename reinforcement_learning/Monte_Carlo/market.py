...
class Market(object):
    ...
    def generate_bookings_for_reserved_night_date(self, reserved_night_date, revenue_manager, person_np,run):
        ...
        for extra_days in range((end_date-start_date).days):
            ...
            #Save each state visited in the episode
        ...
        for i in range(len(wheres)):
            ...
            self.state[wheres[i]][0] += (1/self.state[wheres[i]][1]) * (rev - self.state[wheres[i]][0])
            self.value_DOW[day][wheres[i]][0] += (1/self.value_DOW[day][wheres[i]][1]) * (rev - self.value_DOW[day][wheres[i]][0])
            self.value_month[month][wheres[i]][0] += (1/self.value_month[month][wheres[i]][1]) * (rev - self.value_month[month][wheres[i]][0])
    ...
    def _create_persons(self, reserved_night_date):#,pers_writer):
        ...
        for i in range(len(budgets)):
            ...
            #More output, writes each customers lead day and max budget.Needs pers_writer parsed from run_simulation
